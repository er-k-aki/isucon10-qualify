...
var db, slaveDb *sqlx.DB
var mySQLConnectionData, mySQLSlaveConnectionData *MySQLConnectionEnv
...
func main() {
	...
	slaveDb, err = mySQLSlaveConnectionData.ConnectDB()
	if err != nil {
		e.Logger.Fatalf("DB connection failed : %v", err)
	}
	slaveDb.SetMaxOpenConns(5)
	defer slaveDb.Close()
...
func getChairDetail(c echo.Context) error {
	...
	err = slaveDb.Get(&chair, query, id)
	if err != nil {
...
func searchChairs(c echo.Context) error {
	...
	err = slaveDb.Get(&res.Count, countQuery+searchCondition, params...)
	if err != nil {
	...
	err = slaveDb.Select(&chairs, searchQuery+searchCondition+limitOffset, params...)
	if err != nil {
		if err == sql.ErrNoRows {
...
func getLowPricedChair(c echo.Context) error {
	...
	err := slaveDb.Select(&chairs, query, Limit)
	if err != nil {
		if err == sql.ErrNoRows {
...
func getEstateDetail(c echo.Context) error {
	...
	var estate Estate
	err = slaveDb.Get(&estate, "SELECT * FROM estate WHERE id = ?", id)
	if err != nil {
		if err == sql.ErrNoRows {
...
func searchEstates(c echo.Context) error {
	...
	estates := []Estate{}
	params = append(params, perPage, page*perPage)
	err = slaveDb.Select(&estates, searchQuery+searchCondition+limitOffset, params...)
	if err != nil {
		if err == sql.ErrNoRows {
...
func getLowPricedEstate(c echo.Context) error {
	...
	err := slaveDb.Select(&estates, query, Limit)
	if err != nil {
		if err == sql.ErrNoRows {
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	err = slaveDb.Select(&estates, query, w, h, w, d, h, w, h, d, d, w, d, h, Limit)
	if err != nil {
		if err == sql.ErrNoRows {
			return c.JSON(http.StatusOK, EstateListResponse{[]Estate{}})
		}
...
func searchEstateNazotte(c echo.Context) error {
	...
	// err = slaveDb.Select(&estatesInBoundingBox, query, b.BottomRightCorner.Latitude, b.TopLeftCorner.Latitude, b.BottomRightCorner.Longitude, b.TopLeftCorner.Longitude)
	// if err == sql.ErrNoRows {
	...
	// 	err = slaveDb.Get(&validatedEstate, query, estate.ID)
	// 	if err != nil {
	// 		if err == sql.ErrNoRows {
	// 			continue
	// 		} else {
...
func postEstateRequestDocument(c echo.Context) error {
	...
	err = slaveDb.Get(&estate, query, id)
	if err != nil {
		if err == sql.ErrNoRows {
			return c.NoContent(http.StatusNotFound)
		}
