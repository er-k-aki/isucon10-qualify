...
func NewMySQL2ConnectionEnv() *MySQLConnectionEnv {
	return &MySQLConnectionEnv{
		// Host:     getEnv("MYSQL_HOST", "127.0.0.1"),
		Host:     "192.168.33.12",
		Port:     getEnv("MYSQL_PORT", "3306"),
...
func main() {
	...
	mySQL2ConnectionData = NewMySQL2ConnectionEnv()
	...
	db2, err = mySQL2ConnectionData.ConnectDB()
	if err != nil {
		e.Logger.Fatalf("DB connection failed : %v", err)
	}
...
func initialize(c echo.Context) error {
	...
	for _, p := range paths2 {
		sqlFile, _ := filepath.Abs(p)
		cmdStr := fmt.Sprintf("mysql -h %v -u %v -p%v -P %v %v < %v",
			mySQL2ConnectionData.Host,
			mySQL2ConnectionData.User,
			mySQL2ConnectionData.Password,
			mySQL2ConnectionData.Port,
			mySQL2ConnectionData.DBName,
			sqlFile,
		)
		if err := exec.Command("bash", "-c", cmdStr).Run(); err != nil {
			c.Logger().Errorf("Initialize script error : %v", err)
...
func searchChairs(c echo.Context) error {
	...
	err = db2.Get(&res.Count, countQuery)
	if err != nil {
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	h := chair.Height
	d := chair.Depth
	query = `SELECT id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features, popularity FROM estate WHERE (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) ORDER BY popularity_desc,  id ASC LIMIT ?`
